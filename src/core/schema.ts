...
export const Snapshot = z.strictObject({ schemaVersion: z.literal(1), provider: z.enum(['chatgpt', 'gemini', 'claude']).optional(), conversationId: Id,
  sourceUrl: z.string().max(512), title: z.string().max(512), capturedAt: z.iso.datetime(),
  messages: z.array(z.strictObject({ id: Id, role: z.enum(['user', 'assistant']), kind: z.literal('thinking').optional(), parts: z.array(z.union([
  ...
  const location = conversationLocation(snapshot.sourceUrl);
  if (!location || location.url !== snapshot.sourceUrl || location.id !== snapshot.conversationId || location.provider !== (snapshot.provider ?? 'chatgpt')) context.addIssue({ code: 'custom', message: 'Conversation identity mismatch' });
...
export const SaveRequest = z.strictObject({ requestId: z.uuid(), expectedRevision: z.number().int().nonnegative(), snapshot: Snapshot, exportOptions: ExportOptions.optional() });
...
export const ReceiptSchema = z.strictObject({ requestId: z.uuid(), action: z.enum(['created', 'updated', 'forked', 'unchanged']),
